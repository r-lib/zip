...
[miniz](https://r-lib.github.io/zip/dev/src/miniz.h) C library via R’s
...
[src/init.c](https://r-lib.github.io/zip/dev/src/init.c): `R_zip_zip`,
...
`zip_internal()` in [R/zip.R](https://r-lib.github.io/zip/dev/R/zip.R)
delegates path resolution to `get_zip_data()` in
[R/utils.R](https://r-lib.github.io/zip/dev/R/utils.R), which builds a
data frame with columns `key` (path inside the zip), `file` (path on
disk), and `dir` (logical).
...
in [R/process.R](https://r-lib.github.io/zip/dev/R/process.R) are R6
...
[src/tools/cmdzip.c](https://r-lib.github.io/zip/dev/src/tools/cmdzip.c)
and
[src/tools/cmdunzip.c](https://r-lib.github.io/zip/dev/src/tools/cmdunzip.c)
by [src/Makevars](https://r-lib.github.io/zip/dev/src/Makevars)
alongside the shared library. `zip_process` serialises its parameters to
a binary file read by `cmdzip`.

### Encoding handling

- On Windows, all file paths go through UTF-8 → UTF-16 conversion
  (`zip__utf8_to_utf16` in
  [src/winutils.c](https://r-lib.github.io/zip/dev/src/winutils.c)) to
...
- ZIP filenames without the UTF-8 flag (bit 11 of the general purpose
  ...
  [src/zip.c](https://r-lib.github.io/zip/dev/src/zip.c).
- The R helper `enc2c()`
  ([R/utils.R](https://r-lib.github.io/zip/dev/R/utils.R)) converts
  strings to UTF-8 on Windows and native encoding elsewhere before
  passing them to C.
