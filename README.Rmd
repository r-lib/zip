...
`unzip()` and `zip_list()` also work on HTTP(S) URLs, without downloading
the whole file. This needs the curl package to be installed:
...
* `R_ZIP_PROCESS_FALLBACK`: If set to `TRUE`, `unzip_process()` functions
  will fall back to using an R subprocess, without trying to use the
  cmdunzip executable embedded into the package. This is useful when
